...
export default class Wizard extends Webform {
   ...
   *    - enabledWhenVisited: false (default) determines if the breadcrumb bar is clickable or not for visited tabs
   */
  constructor() {
    let element, options;
  ...
  init() {
    ...
    this.options.enabledWhenVisited = this.options.enabledWhenVisited || false;
  ...
  attach(element) {
    ...
    if (this.options.enabledWhenVisited && !this.enabledIndex) {
      this.enabledIndex = this.options.readOnly || this.editMode ? this.pages?.length - 1 : 0;
    }
  ...
  attachHeader() {
    if (this.isBreadcrumbClickable()) {
      this.refs[`${this.wizardKey}-link`].forEach((link, index) => {
        console.log('this.options.enabledWhenVisited', this.options.enabledWhenVisited);
        if (!this.options.enabledWhenVisited || index <= this.enabledIndex) {
  ...
  nextPage() {
    ...
    if (this.checkValidity(this.submission.data, true, this.submission.data, true)) {
      ...
      return this.beforePage(true).then(() => {
        return this.setPage(this.getNextPage()).then(() => {
          if (this.options.enabledWhenVisited && this.enabledIndex < this.page) {
  ...
  setValue(submission, flags = {}) {
    ...
    if (this.options.enabledWhenVisited && !this.editMode && submission._id) {
      this.editMode = true;
      this.redraw();
    }
